...
function TodoComponent() {
	...
	function retriveTodo() {
		if (id != -1) {
			getTodo(username, id).then((responce) => {
	...
	function onSubmit(values) {
		...
		if (id != -1) {
			updateTodo(username, id, todo).then((responce) => {
				setDescription(responce.data.description);
				setTargetDate(responce.data.targetDate);
			});
		} else {
	...
	function validate(values) {
		let errors = {};
		if (values.description.length < 5) errors.description = 'description should be more than 5 characters';
		return errors;
	}
